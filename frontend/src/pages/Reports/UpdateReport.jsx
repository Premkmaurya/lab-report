...
export const UpdateReport = () => {
  ...
  const { patientId, assignedTests } = location.state || {};
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [patientData, reportData, testData] = await Promise.all([
          patientService.getReportsByPatientId(patientId),
        ...
        setSubmitError(
          err.response?.data?.message ||
            "Failed to load patient and test data.",
        );
      } finally {
        setLoading(false);
      }
  ...
  const availableTests = useMemo(() => {
    ...
    return allTests.filter((test) => test?.name && test.name.trim().length > 0);
  }, [allTests]);
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div>
        ...
        <h1 className="font-martinaplantijn text-4xl text-ink-navy">
          Update <span className="italic font-light">Patient Report</span>
        </h1>
        <p className="font-inter text-stone text-sm mt-1">
          {patient?.name
            ? `For ${patient.name}`
            : "Select the tests to include in this report."}
        </p>
      </div>
